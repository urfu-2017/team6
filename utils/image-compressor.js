...
function ImageCompressor(canvas, img) {
...
ImageCompressor.prototype = {
    ...
    run(src, callback) {
        const settings = COMPRESSOR_DEFAULTS
        this.settings = JSON.parse(JSON.stringify(COMPRESSOR_DEFAULTS))
    ...
    fitWidth() {
        let compressedHeight = this.settings.toWidth / this.naturalAR,
            offsetX = 0,
    ...
    fitHeight() {
        let compressedWidth = this.settings.toHeight * this.naturalAR,
            offsetY = 0,
